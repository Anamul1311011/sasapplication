...
@section('main_content')
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <h2>Awesome Applicatins</h2>
                <p><a href="{{route('welcome')}}">Home</a> /  {{$awesome_app->title}}</p>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="row">
                        <div class="col-sm-6 fadesimple wow slideInLeft">
                            <div class="default-title">
                              <input type="hidden" name="product_title" value="{{$awesome_app->title}}">
                              <input type="hidden" name="product_id" value="{{$awesome_app->id}}">
                              <input type="hidden" name="main_category" value="application">
                                <h3>{{$awesome_app->title}}</h3>
                                <h5 class="w-100 text-left">{!! str_limit($awesome_app->description,150) !!}</h5>
                            </div>
                            <input type="hidden" name="product_price" value="{{$awesome_app->price}}">
                            <h5 class="w-100 text-left">Price: <font class="text-danger">{{$awesome_app->price}}</font></h5>
                            {{-- <a href="{{ url('add/cart') }}/{{ $awesome_app->id }}" class="btn button btn-2 red btn-lg">Buy Now</a> --}}
                            <button class="btn button btn-sm red" type="submit" name="button">Buy Now</button>
                        </div>
                        <div class="col-sm-6 fadesimple">
                          <input type="hidden" name="product_image" value="{{$awesome_app->image}}">
                            <img alt="Dedicated Hosting" src="{{asset('/storage')}}/{{$awesome_app->image}}" class="img-responsive">
                        </div>
                    </div>
                </div>
                </form>
    ...
    @foreach($all_apps as $hosting)
        @if($hosting->id==$awesome_app->id)
            @continue
        @endif
