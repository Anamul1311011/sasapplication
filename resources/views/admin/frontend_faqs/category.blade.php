...
<div class="col-md-12">
    ...
    <!-- DATA TABLE -->
    <h3 class="title-5 m-b-35 pl-3">data table of frontend faq categories</h3>
    @if($errors->any())
        <script>
            swal({
    ...
    <div class="table-data__tool">
        ...
        <div class="table-data__tool-right">
            <button class="au-btn au-btn-icon au-btn--green au-btn--small" data-toggle="modal" data-target="#adding">
                <i class="zmdi zmdi-plus"></i>add frontend faq categories
            </button>
            <!-- Modal -->
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content rounded-0 border-0">
                        <div class="modal-header rounded-0 border-0 bg-success">
                            <h5 class="modal-title text-white" id="exampleModalCenterTitle">Add Frontend FAQ Category</h5>
                            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{route('insert_frontend_faq_category')}}" method="post">
                            @csrf
            ...
            <table class="table table-hover text-center">
                ...
                <tbody>
                    ...
                    <tr class="tr-shadow bg-white">
                        ...
                        <td style="vertical-align:middle;" class="py-4">{!! str_limit($category->description,100) !!}</td>
                                ...
                                <div class="modal fade" id="edit{{$category->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content rounded-0 border-0">
                                            ...
                                            <form action="{{route('update_frontend_faq_category')}}" method="post">
                                                @csrf
                                                <div class="modal-body text-left">
                                                    ...
                                                    <div class="form-group">
                                                        <label class="font-weight-bold">Description</label>
                                                        <textarea type="text" name="Udescription" id="Udescription{{$category->id}}" class="form-control{{ $errors->has('Udescription') ? ' is-invalid' : '' }}">{!! $category->description !!}</textarea>
