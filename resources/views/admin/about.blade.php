...
<div class="col-md-12">
    ...
    <!-- DATA TABLE -->
    <h3 class="title-5 m-b-35 pl-3">data table of About Us</h3>
    @if($errors->any())
        <script>
            swal({
    ...
    <div class="table-data__tool">
        ...
        <div class="table-data__tool-right">
            <button class="au-btn au-btn-icon au-btn--green au-btn--small" data-toggle="modal" data-target="#adding">
                <i class="zmdi zmdi-plus"></i>add new about 
            </button>
            <!-- Modal -->
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                    <div class="modal-content rounded-0 border-0">
                        <div class="modal-header rounded-0 border-0 bg-success">
                            <h5 class="modal-title text-white" id="exampleModalCenterTitle">Add About Us</h5>
                            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{route('insert_aboutUs')}}" enctype="multipart/form-data" method="post">
                            @csrf
        ...
        <div class="col-lg-12">
            <table class="table table-hover text-center">
                <thead class="thead-light">
                    <tr>
                        <th style="vertical-align:middle;" class="text-center">About Title</th>
                ...
                @forelse($abouts as $about)
                    <tr class="tr-shadow bg-white">
                        <td style="vertical-align:middle;" class="py-4">{{$about->title}}</td>
                        <td style="vertical-align:middle; font-size:25px;" class="py-4">{!! $about->icon !!}</td>
                        <td style="vertical-align:middle;" class="py-4">
                            <img src="{{asset('/storage')}}/{{$about->image}}" alt="No About Image Found" width="50" height="50">
                        </td>
                        <td style="vertical-align:middle;" class="py-4">
                           {!! str_limit($about->description,45) !!}
                        </td>
                        <td style="vertical-align:middle;" class="py-4">
                            <div class="table-data-feature">
                                <button class="item" data-toggle="modal" data-target="#edit{{$about->id}}" title="Edit">
                                    <i class="zmdi zmdi-edit"></i>
                                </button>
                                <div class="modal fade" id="edit{{$about->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                        <div class="modal-content rounded-0 border-0">
                                            <div class="modal-header rounded-0 border-0 bg-primary">
                                                <h5 class="modal-title text-white" id="exampleModalCenterTitle">Edit About Information</h5>
                                                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="{{route('update_aboutUs')}}" enctype="multipart/form-data" method="post">
                                                @csrf
                                                <div class="modal-body text-left">
                                                    <div class="form-group">
                                                        <input type="hidden" name="Uid" value="{{$about->id}}">
                                                        <label class="font-weight-bold">Title</label>
                                                        <input type="text" name="Utitle" class="form-control{{ $errors->has('Utitle') ? ' is-invalid' : '' }}" value="{{$about->title}}">
                                                    ...
                                                    <div class="form-group">
                                                        <label class="font-weight-bold">Icon</label>
                                                        <input type="text" name="Uicon" class="form-control{{ $errors->has('Uicon') ? ' is-invalid' : '' }}" value="{{$about->icon}}">
                                                        @if ($errors->has('Uicon'))
                                                            <span class="invalid-feedback" role="alert">
                                                    ...
                                                    <div class="form-group">
                                                        <label class="font-weight-bold">Description</label>
                                                        <textarea name="Udescription" id="Udescription{{$about->id}}" class="form-control{{ $errors->has('Udescription') ? ' is-invalid' : '' }}">{!! $about->description !!}</textarea>
                                                        ...
                                                        @endif
                                                        <script>
                                                            CKEDITOR.replace('Udescription{{$about->id}}')
                                                        </script>
                                                    </div>
                                                </div>
                                                <div class="modal-footer">
                                ...
                                <button onclick="testing{{$about->id}}()" data-toggle="tooltip" data-placement="top" class="item" title="Delete">
                                    <i class="zmdi zmdi-delete"></i>
                                </button>
                                <script>
                                    function testing{{$about->id}}() {
                                        swal({
                                            ...
                                            .then((willDelete) => {
                                                if (willDelete) {
                                                    location.href = "{{route('delete_aboutUs',$about->id)}}"
                                                }
                                        });
                                    }
                                </script>
                    ...
                    <tr class="spacer"></tr>
                @empty
                    <tr><td colspan="5">There is no information of about us found</td></tr>
                @endforelse
                </tbody>
            </table>
            {{ $abouts->links() }}
