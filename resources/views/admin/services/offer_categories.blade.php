...
<div class="col-md-12">
    ...
    <!-- DATA TABLE -->
    <h3 class="title-5 m-b-35 pl-3">data table of services area</h3>
    @if($errors->any())
        <script>
            swal({
    ...
    <div class="table-data__tool">
        ...
        <div class="table-data__tool-right">
            <button class="au-btn au-btn-icon au-btn--green au-btn--small" data-toggle="modal" data-target="#adding">
                <i class="zmdi zmdi-plus"></i>add services area
            </button>
            <!-- Modal -->
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                    <div class="modal-content rounded-0 border-0">
                        <div class="modal-header rounded-0 border-0 bg-success">
                            <h5 class="modal-title text-white" id="exampleModalCenterTitle">Add Services area</h5>
                            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{route('insert_offer_categories')}}" enctype="multipart/form-data" method="post">
                            @csrf
        ...
        <div class="col-lg-12">
            <table class="table table-hover text-center" id="datatable">
                <thead class="thead-light">
                    <tr>
                        <th style="vertical-align:middle;" class="text-center">Service Category</th>
                ...
                @forelse($offers as $offer)
                    <tr class="tr-shadow bg-white">
                        ...
                        <td style="vertical-align:middle; font-size:25px;" class="py-4">{!! $offer->icon !!}</td>
                        <td style="vertical-align:middle;" class="py-4">
                            {!! str_limit($offer->description,55) !!}
                        </td>
                        <td style="vertical-align:middle;" class="py-4">
                                    ...
                                    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                        <div class="modal-content rounded-0 border-0">
                                            <div class="modal-header rounded-0 border-0 bg-primary">
                                                <h5 class="modal-title text-white" id="exampleModalCenterTitle">Edit Services Offer Categories</h5>
                                                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="{{route('update_offer_categories')}}" method="post">
                                                @csrf
                                                <div class="modal-body text-left">
                                                    ...
                                                    <div class="form-group">
                                                        <label class="font-weight-bold">Icon</label>
                                                        <input type="text" name="Uicon" class="form-control{{ $errors->has('Uicon') ? ' is-invalid' : '' }}" value="{{$offer->icon}}">
                                                        @if ($errors->has('Uicon'))
                                                            <span class="invalid-feedback" role="alert">
                                ...
                                <script>
                                    function testing{{$offer->id}}() {
                                        swal({
                                            ...
                                            .then((willDelete) => {
                                                if (willDelete) {
                                                    location.href = "{{route('delete_offer_category',$offer->id)}}"
                                                }
                                        });
                                    }
                                </script>
                    ...
                    <tr class="spacer"></tr>
                @empty
                    <tr><td colspan="4">There is no service offer category found</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
