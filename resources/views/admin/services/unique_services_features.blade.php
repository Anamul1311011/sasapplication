...
<div class="col-md-12">
    ...
    <!-- DATA TABLE -->
    <h3 class="title-5 m-b-35 pl-3">data table of different services features</h3>
    @if($errors->any())
        <script>
            swal({
    ...
    <div class="table-data__tool">
        ...
        <div class="table-data__tool-right">
            <button class="au-btn au-btn-icon au-btn--green au-btn--small" data-toggle="modal" data-target="#adding">
                <i class="zmdi zmdi-plus"></i>add different service features
            </button>
            <!-- Modal -->
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content rounded-0 border-0">
                        <div class="modal-header rounded-0 border-0 bg-success">
                            <h5 class="modal-title text-white" id="exampleModalCenterTitle">Add different Service Features</h5>
                            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{route('insert_unique_services_features')}}" method="post">
                            @csrf
            ...
            <table class="table table-hover text-center">
                ...
                <tbody>
                    ...
                    <tr class="tr-shadow bg-white">
                        <td style="vertical-align:middle;" class="py-4">{{$feature->rel_to_services->service}}</td>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content rounded-0 border-0">
                                            <div class="modal-header rounded-0 border-0 bg-primary">
                                                <h5 class="modal-title text-white" id="exampleModalCenterTitle">Edit Unique Service Features</h5>
                                                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="{{route('update_unique_services_features')}}" method="post">
                                                @csrf
                                                <div class="modal-body text-left">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="Uservice" class="form-control">
                                                            <option value="{{$feature->service}}">{{$feature->rel_to_services->service}}</option>
                                                            @foreach($services as $service)
                                                                @if($feature->service==$service->id)
                                                                    @continue
                                                                @endif
                                                                <option value="{{$service->id}}">{{$service->service}}</option>
                                                            @endforeach
                                ...
                                <script>
                                    function testing{{$feature->id}}() {
                                        swal({
                                            ...
                                            .then((willDelete) => {
                                                if (willDelete) {
                                                    location.href = "{{route('delete_unique_services_features',$feature->id)}}"
                                                }
                                        });
                                    }
                                </script>
                    ...
                    <tr class="spacer"></tr>
                @empty
                    <tr><td colspan="3">There is no unique service feature found</td></tr>
                @endforelse
                </tbody>
            </table>
            {{ $features->links() }}
