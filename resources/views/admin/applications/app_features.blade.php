...
<div class="col-md-12">
    ...
    <!-- DATA TABLE -->
    <h3 class="title-5 m-b-35 pl-3">data table of application features</h3>
    @if($errors->any())
        <script>
            swal({
    ...
    <div class="table-data__tool">
        ...
        <div class="table-data__tool-right">
            <button class="au-btn au-btn-icon au-btn--green au-btn--small" data-toggle="modal" data-target="#adding">
                <i class="zmdi zmdi-plus"></i>add application features
            </button>
            <!-- Modal -->
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content rounded-0 border-0">
                        <div class="modal-header rounded-0 border-0 bg-success">
                            <h5 class="modal-title text-white" id="exampleModalCenterTitle">Add Application Feature</h5>
                            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{route('insert_app_features')}}" method="post">
                            @csrf
                            <div class="modal-body">
                                <div class="form-group">
                                    <label class="font-weight-bold">Application Name</label>
        ...
        <div class="col-lg-12">
            <table class="table table-hover text-center">
                <thead class="thead-light">
                    <tr>
                        <th style="vertical-align:middle;" class="text-center">Application Name</th>
                ...
                @forelse($features as $feature)
                    @if(($feature->application)==($feature->rel_to_apps->id))
                    <tr class="tr-shadow bg-white">
                        <td style="vertical-align:middle;" class="py-4">
                            {{$feature->rel_to_apps->title}}
                        </td>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content rounded-0 border-0">
                                            <div class="modal-header rounded-0 border-0 bg-primary">
                                                <h5 class="modal-title text-white" id="exampleModalCenterTitle">Edit Application Feature</h5>
                                                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="{{route('update_app_features')}}" method="post">
                                                @csrf
                                                <div class="modal-body text-left">
                                                    <div class="form-group">
                                                        <input type="hidden" name="Uid" value="{{$feature->id}}">
                                                        <label class="font-weight-bold">Application Name</label>
                                                        <select name="Uapplication" class="form-control">
                                                            <option value="{{$feature->application}}">{{$feature->application}}</option>
                                                            @foreach($applications as $application)
                                                                @if($feature->application==$application->id)
                                                                    @continue
                                                                @endif
                                                                <option value="{{$application->id}}">{{$application->title}}</option>
                                                            @endforeach
                                ...
                                <script>
                                    function testing{{$feature->id}}() {
                                        swal({
                                            ...
                                            .then((willDelete) => {
                                                if (willDelete) {
                                                    location.href = "{{route('delete_app_features',$feature->id)}}"
                                                }
                                        });
                                    }
                                </script>
                    ...
                    @endif
                @empty
                    <tr><td colspan="3">There is no application feature found</td></tr>
                @endforelse
                </tbody>
            </table>
            {{ $features->links() }}
