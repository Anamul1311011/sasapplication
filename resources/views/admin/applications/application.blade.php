...
<div class="col-md-12">
    ...
    <!-- DATA TABLE -->
    <h3 class="title-5 m-b-35 pl-3">data table of applications</h3>
    @if($errors->any())
        <script>
            swal({
    ...
    <div class="table-data__tool">
        ...
        <div class="table-data__tool-right">
            <button class="au-btn au-btn-icon au-btn--green au-btn--small" data-toggle="modal" data-target="#adding">
                <i class="zmdi zmdi-plus"></i>add applications
            </button>
            <!-- Modal -->
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                    <div class="modal-content rounded-0 border-0">
                        <div class="modal-header rounded-0 border-0 bg-success">
                            <h5 class="modal-title text-white" id="exampleModalCenterTitle">Add Application</h5>
                            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{route('insert_applications')}}" enctype="multipart/form-data" method="post">
                            @csrf
                            <div class="modal-body">
                                <div class="form-group">
                                    <label class="font-weight-bold">Application Category</label>
                                ...
                                <div class="form-group">
                                    <label class="font-weight-bold">Application Title</label>
            ...
            <table class="table table-hover text-center">
                ...
                @forelse($applications as $application)
                    <tr class="tr-shadow bg-white">
                        <td style="vertical-align:middle;" class="py-4">{{$application->rel_to_categories->category}}</td>
                        ...
                        <td style="vertical-align:middle;" class="py-4">{!! $application->icon !!}</td>
                        <td style="vertical-align:middle;" class="py-4">{!! str_limit($application->description,30) !!}</td>
                        <td style="vertical-align:middle;" class="py-4">
                                    ...
                                    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                        <div class="modal-content rounded-0 border-0">
                                            <div class="modal-header rounded-0 border-0 bg-primary">
                                                <h5 class="modal-title text-white" id="exampleModalCenterTitle">Edit Application</h5>
                                                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="{{route('update_applications')}}" enctype="multipart/form-data" method="post">
                                                @csrf
                                                <div class="modal-body text-left">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="Ucategory" class="form-control">
                                                            <option value="{{$application->category}}">{{$application->rel_to_categories->category}}</option>
                                                            @foreach($categories as $category)
                                                                @if($application->category==$category->id)
                                                                    @continue
                                                                @endif
                                                                <option value="{{$category->id}}">{{$category->category}}</option>
                                                            @endforeach
                                                    ...
                                                    <div class="form-group">
                                                        <label class="font-weight-bold">Application Title</label>
                                                        <input type="text" name="Utitle" class="form-control{{ $errors->has('Utitle') ? ' is-invalid' : '' }}" value="{{$application->title}}">
                                                    ...
                                                    <div class="form-group">
                                                        <label class="font-weight-bold">Description</label>
                                                        <textarea type="text" name="Udescription" id="Udescription{{$application->id}}" class="form-control{{ $errors->has('Udescription') ? ' is-invalid' : '' }}">{!! $application->description !!}</textarea>
                                ...
                                <script>
                                    function testing{{$application->id}}() {
                                        swal({
                                            ...
                                            .then((willDelete) => {
                                                if (willDelete) {
                                                    location.href = "{{route('delete_applications',$application->id)}}"
                                                }
                                        });
                                    }
                                </script>
