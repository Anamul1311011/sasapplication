...
<div class="col-md-12">
    ...
    <!-- DATA TABLE -->
    <h3 class="title-5 m-b-35 pl-3">data table of application offers</h3>
    @if($errors->any())
        <script>
            swal({
    ...
    <div class="table-data__tool">
        ...
        <div class="table-data__tool-right">
            <button class="au-btn au-btn-icon au-btn--green au-btn--small" data-toggle="modal" data-target="#adding">
                <i class="zmdi zmdi-plus"></i>add application offers
            </button>
            <!-- Modal -->
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                    <div class="modal-content rounded-0 border-0">
                        <div class="modal-header rounded-0 border-0 bg-success">
                            <h5 class="modal-title text-white" id="exampleModalCenterTitle">Add Application Offer</h5>
                            <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{route('insert_application_offers')}}" method="post">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="offer" class="form-control{{ $errors->has('offer') ? ' is-invalid' : '' }}" value="{{old('offer')}}">
                                    @if ($errors->has('offer'))
                                        <span class="invalid-feedback" role="alert">
            ...
            <table class="table table-hover text-center">
                <thead class="thead-light">
                    <tr>
                        ...
                        <th style="vertical-align:middle;" class="text-center">Offer</th>
                ...
                @forelse($offers as $offer)
                    <tr class="tr-shadow bg-white">
                        ...
                        <td style="vertical-align:middle;" class="py-4">{{$offer->offer}}</td>
                        <td style="vertical-align:middle;" class="py-4">{{$offer->price}}</td>
                        <td style="vertical-align:middle;" class="py-4">{!! str_limit($offer->description,150) !!}</td>
                                    ...
                                    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                        <div class="modal-content rounded-0 border-0">
                                            <div class="modal-header rounded-0 border-0 bg-primary">
                                                <h5 class="modal-title text-white" id="exampleModalCenterTitle">Edit Application Offer</h5>
                                                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="{{route('update_application_offers')}}" method="post">
                                                @csrf
                                                <div class="modal-body text-left">
                                                    ...
                                                    <div class="form-group">
                                                        <label class="font-weight-bold">Offer</label>
                                                        <input type="text" name="Uoffer" class="form-control{{ $errors->has('Uoffer') ? ' is-invalid' : '' }}" value="{{$offer->offer}}">
                                                        @if ($errors->has('Uoffer'))
                                                            <span class="invalid-feedback" role="alert">
                                                                <strong>{{ $errors->first('Uoffer') }}</strong>
                                                            </span>
                                                        @endif
                                                    </div>
                                                    <div class="form-group">
                                ...
                                <script>
                                    function testing{{$offer->id}}() {
                                        swal({
                                            ...
                                            .then((willDelete) => {
                                                if (willDelete) {
                                                    location.href = "{{route('delete_application_offer',$offer->id)}}"
                                                }
                                        });
                                    }
                                </script>
                    ...
                    <tr class="spacer"></tr>
                @empty
                    <tr><td colspan="5">There is no application offer found</td></tr>
                @endforelse
                </tbody>
            </table>
            {{ $offers->links() }}
